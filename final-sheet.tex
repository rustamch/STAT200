...
\item Construct the null-model: $\mathbf{N}(\mu,\frac{s}{\sqrt{n}})$
\item Find the test statistic: \begin{math}t_o = \frac{\bar{y_1}-\bar{y_2}-\Delta_o}{SD(\bar{y_1}-\bar{y_2})}\end{math}
\item Using R compute the p-value:
\begin{itemize}
    \item One-sided hypothesis : \begin{math}\textrm{pt}(t_o, \textrm{df=min}(n_1-1, n_2-1))\end{math}
    \item Two-sided hypothesis : 2 $\cdot$ \begin{math}\textrm{pt}(t_o, \textrm{df=min}(n_1-1, n_2-1))\end{math}
\end{itemize}
\item If the p-value is less than $\alpha$ - reject the null-hypothesis. 
    Otherwise, you fail to reject the null-hypothesis.
\end{itemize}
...
\begin{itemize}
    ...
    \item N = number of subjects in total.
    \item SSTo = Sum of Squares Total.
    \item SSTo = SSTr + SSEr.\\
    \includegraphics{ANOVA.png}
    \item After filling the table with the values and finding F statistic, you can decide to reject the null-hypothesis or not based on two methods:
    \begin{itemize}
        \item Method 1: 
        \begin{enumerate}
            \item Compute critical value using R : qf($1 - \alpha$, k - 1, N - k, lower.tail=TRUE) 
            \item If F-statistic is greater than critical value - reject the null-hypothesis. Otherwise, you fail to reject the null-hypothesis.
        \end{enumerate}
        \item Method 2:
        \begin{enumerate}
            \item Compute the pvalue using R: pf($F$, k - 1, N - k, lower.tail=FALSE)
            \item If pvalue is less than $\alpha$ - reject the null-hypothesis. Otherwise, you fail to reject the null-hypothesis.
        \end{enumerate}
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item c(kj...) = all observations from the table given where you enter the data for each row from left to right.
    \item Enter your data: matrix($c(k_ij\dots)$, ncol=n, byrow=TRUE) 
    \item Using R compute the p-value and test-statistic: chisq.test(sample.data, correct=FALSE)
    \item If the p-value is less than $\alpha$ - reject the null-hypothesis. Otherwise, you fail to reject the null-hypothesis.
    \item Example:\\
    \includegraphics[scale = 0.5]{chisq.png}
    \begin{enumerate}
        \item Enter data into R: matrix(c(11,4,2,6), ncol=2, byrow=TRUE)
        \item Using chisq function compute test-statistic: chisq.test(sample.data, correct=FALSE)\\
        \includegraphics{chisq_results.png}
        \item At 5$\%$ confidence level we would reject the null-hypothesis, because the p-value is less than 5$\%$.
    \end{enumerate}
    % HUH
\end{itemize}
\end{document}
